...
#define UAVCAN_EQUIPMENT_ACTUATOR_STATUS_MAX_SIZE 15
#define UAVCAN_EQUIPMENT_ACTUATOR_STATUS_SIGNATURE (0x94949098635C458ULL)
...
void _uavcan_equipment_actuator_Status_encode(uint8_t* buffer, uint32_t* bit_ofs, struct uavcan_equipment_actuator_Status* msg, bool tao) {
    ...
    *bit_ofs += 8;
    canardEncodeScalar(buffer, *bit_ofs, 32, &msg->error_status);
    *bit_ofs += 32;
    {
    ...
    *bit_ofs += 16;
    {
        uint16_t float16_val = canardConvertNativeFloatToFloat16(msg->velocity);
        canardEncodeScalar(buffer, *bit_ofs, 16, &float16_val);
    }
    *bit_ofs += 16;
    {
        uint16_t float16_val = canardConvertNativeFloatToFloat16(msg->torque);
        canardEncodeScalar(buffer, *bit_ofs, 16, &float16_val);
    }
    *bit_ofs += 16;
    {
        uint16_t float16_val = canardConvertNativeFloatToFloat16(msg->control_power);
        canardEncodeScalar(buffer, *bit_ofs, 16, &float16_val);
    }
    *bit_ofs += 16;
...
bool _uavcan_equipment_actuator_Status_decode(const CanardRxTransfer* transfer, uint32_t* bit_ofs, struct uavcan_equipment_actuator_Status* msg, bool tao) {
    ...
    *bit_ofs += 8;
    ...
    canardDecodeScalar(transfer, *bit_ofs, 32, false, &msg->error_status);
    *bit_ofs += 32;
    ...
    *bit_ofs += 16;

    {
        uint16_t float16_val;
        canardDecodeScalar(transfer, *bit_ofs, 16, true, &float16_val);
        msg->velocity = canardConvertFloat16ToNativeFloat(float16_val);
    }
    *bit_ofs += 16;

    {
        uint16_t float16_val;
        canardDecodeScalar(transfer, *bit_ofs, 16, true, &float16_val);
        msg->torque = canardConvertFloat16ToNativeFloat(float16_val);
    }
    *bit_ofs += 16;

    {
        uint16_t float16_val;
        canardDecodeScalar(transfer, *bit_ofs, 16, true, &float16_val);
        msg->control_power = canardConvertFloat16ToNativeFloat(float16_val);
    }
    *bit_ofs += 16;
